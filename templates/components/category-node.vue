<div v-bind="$attrs" class="app-category-node">
  <div v-context-menu="contextMenu != null ? contextMenu(item) : null" class="_container" :class="{'_selected': $controller && $controller.selected && $controller.selected.id === item.id}" v-on:click="select" v-on:contextmenu="select">
    ...
    <span v-if="sufix != null && sufix.caption(item) != null"  class="_sufix pull-right"><span>${sufix.caption(item)}</span><i class="fa" :class="[sufix.icon]"></i></span>
  </div>
