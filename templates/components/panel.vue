<div class="app-panel-container"
     ...
     :style="{'max-width':maxWidth + '!important','min-width':visible ? (minWidth + '!important') : null, width: finalWidth, 'padding-left': offset === 'left' || offset === 'both' ? '50px' : null, 'padding-right': offset === 'right' || offset === 'both' ? '50px' : null }"
     :class="{'app-panel-over-left' : (over === 'true' || over === true) && isLeft, 'app-panel-over-right' : (over === 'true' || over === true) && isRight, 'app-panel-container-body': isBody,
  ...
  <div class="app-panel-container-table" :class="{'app-panel-shadow' : (over === 'true' || over === true) && visible === true}">
    ...
    <div class="app-panel-content-body">
      ...
      <div v-if="visible" ref="body" v-on:scroll="updateScroll" class="_content app-scrollable pos-rlt" :class="{'hidden-scroller': noScrollbar === 'true' || noScrollbar === true, 'p-a': (noPadding !== 'true' &&  noPadding !== true)}" v-bind:style="{'border-left': border ? border.left : '', 'border-right': border ? border.right : '', 'background-color':color, 'padding-top' : title != null ? '0px!important' : null}" style="padding-bottom: 0px!important">
        <slot name="content" v-bind:actions="actions"></slot>
      </div>
      
      <transition name="fade">
        <div class="_more-icon" v-if="visible && scrollValue != null && scrollValue > 5">
