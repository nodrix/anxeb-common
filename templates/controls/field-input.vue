<div v-bind="$attrs" class="app-field-parent">
	...
	<div class="md-form-group app-field-input-container" :field-name="name" v-if="type !== 'calendar'">
		<input v-fill-focus="autoSelect === 'true' || autoSelect === true" :class="{'text-right':align==='right'}" :style="{color:valueColor, 'font-weight': valueWeight}" :readonly="readonly === 'true' || readonly === true" v-if="type !== 'area'" :type="type || 'text'" step="any" v-bind:autocomplete="type === 'password'" class="md-input" :value="post_value" v-on="listener" :id="id" :required="required !== undefined" v-focus="focus === undefined ? false : focus" :maxlength="maxLength">
		<textarea :class="{'text-right':align==='right'}" :style="{height: height, color:valueColor, 'font-weight': valueWeight}" :readonly="readonly === 'true' || readonly === true" v-if="type === 'area'" class="md-input" :value="value" v-on="listener" :id="id" :required="required !== undefined" v-focus="focus === undefined ? false : focus" :rows="rows" :maxlength="maxLength"></textarea>
